...
-- TOAD format - run script (don't let TOAD separate into tabs) and enter SQL ID at the prompt.
...
ttitle "Recent ash history with wait objs"
...
ttitle "Plans (historical)"

select * from table(dbms_xplan.display_awr('&&sql_id', null, null, 'ADVANCED'));

ttitle "Plan (current)"

select * from table(dbms_xplan.display_cursor(trim('&&sql_id'), null, 'ADVANCED'));

ttitle "Last captured binds"
